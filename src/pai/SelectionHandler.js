...
class SelectHandler extends Handler {
  ...
  onCellMouseDown(cell, view, e, localX, localY) {
    ...
    if (this.isNode(cell) && !this.isGroup(cell)
      && !this.isRemark(cell) && view.isPortElem(e.target)) {
  ...
  getPreviewBounds() {
    ...
    utils.forEach(this.movingCells, (node) => {
      if (node.isNode()) {
  ...
  autoScrollPreview(direction) {
    ...
    if (this.isParentScrollable()) {
      ...
      if (scrolled) {
        ...
        this.updatePreview();
        this.scrollTimer = setTimeout(this.autoScrollPreview.bind(this, direction),
  ...
  updatePreview(resize) {
    ...
    if (bounds) {
      ...
      if (resize) {
        ...
        if (this.movingCells.length === 1 && !this.isGroup(this.movingCells[0])
          && !this.isRemark(this.movingCells[0])) {
  ...
  autoScrollSelectionRect(localX, localY) {
    ...
    if (this.isParentScrollable()) {
      ...
      if (scrolled) {
        ...
        scrollParent.scrollLeft = scrollLeft;
        this.updateSelectionRect();
        this.scrollTimer = setTimeout(this.autoScrollSelectionRect.bind(this, localX, localY),
          this.options.scrollDelay);
      }
    }
  ...
  clearSelection() {
    ...
    utils.forEach(this.selectedCells, (cell) => {
  ...
  updateNodesPosition(cells = [], dx = 0, dy = 0) {
    ...
    this.isGroup(parentNode) && !utils.some(groups, group => parentNode === group);
